...
/**
 * A domain generator for basic grid worlds. This domain generator allows for the creation of arbitrarily sized grid worlds with user
 * defined layouts. The grid world supports classic north, south, east, west movement actions that may be either deterministic or stochastic
 * with user defined stochastic failures.
 * <p>
 * The domain is an {@link OODomain} that consists of only two object classes: an agent class and a location class, each of which is defined
 * by and x and y position. Locations also have an attribute defining which type of location it is.
 * <p>
 * Walls are not considered objects. Instead walls are considered part of the transition dynamics. There are 2 types of walls supported.
 * Walls that are more like obstacles and occupy an entire cell of the map and 1D walls. 1D walls are specified as either a horizontal wall
 * on the north side of a cell or a vertical wall on the east side of the wall. A cell may also have a 1D north and east wall in it. The
 * type of wall for each cell is specified by a 2D int matrix provided to the constructor. Cells in the matrix with a 0 are clear of any
 * walls and obstacle; 1s indicate a full cell obstacle; 2s a 1D north wall; 3s a 1D east wall; and 4s indicate that the cell has both a 1D
 * north wall and 1D east wall.
 * <p>
 * Note that if you change the stochastic transition dynamics or the map of the domain generator *after* generating a domain with
 * {@link #generateDomain()}, the previously generated domain will use the settings prior to its generation. To use the new settings, you
 * will need to generate a new domain object.
 * <p>
 * There are five propositional functions supported: atLocation(agent, location), wallToNorth(agent), wallToSouth(agent), wallToEast(agent),
 * and wallToWest(agent).
 *
...
public class GridWorldDomain implements DomainGenerator {
  ...
  /**
   * The wall map where the first index is the x position and the second index is the y position. Values of 1 indicate a wall is there,
   * values of 0 indicate an empty cell
   */
  protected int[][] map;

  /**
   * Matrix specifying the transition dynamics in terms of movement directions. The first index indicates the action direction attempted
   * (ordered north, south, east, west) the second index indicates the actual resulting direction the agent will go (assuming there is no
   * wall in the way). The value is the probability of that outcome. The existence of walls does not affect the probability of the direction
   * the agent will actually go, but if a wall is in the way, it will affect the outcome. For instance, if the agent selects north, but
   * there is a 0.2 probability of actually going east and there is a wall to the east, then with 0.2 probability, the agent will stay in
   * place.
   */
  ...
  /**
   * Sets the domain to use probabilistic transitions. Agent will move in the intended direction with probability probSucceed. Agent will
   * move in a random direction with probability 1 - probSucceed
   *
  ...
  /**
   * Will set the movement direction probabilities based on the action chosen. The index (0,1,2,3) indicates the direction
   * north,south,east,west, respectively and the matrix is organized by transitionDynamics[selectedDirection][actualDirection]. For
   * instance, the probability of the agent moving east when selecting north would be specified in the entry transitionDynamics[0][2]
   *
   * @param transitionDynamics entries indicate the probability of movement in the given direction (second index) for the given action
  ...
  /**
   * Will set the map of the world to the classic Four Rooms map used the original options work (Sutton, R.S. and Precup, D. and Singh, S.,
   * 1999).
   */
  ...
  /**
   * Creates a sequence of 1D north walls spanning the specified start and end x coordinates. If any of the cells spanned already have a
   * east wall set in that location, then the cell is set to have both an east wall and a north wall.
   *
   * @param xi The starting x coordinate of the wall
   * @param xf The ending x coordinate of the wall
   * @param y The y coordinate of the wall
  ...
  /**
   * Creates a sequence of 1D east walls spanning the specified start and end y coordinates. If any of the cells spanned already have a 1D
   * north wall set in that location, then the cell is set to have both a north wall and an east wall.
   *
   * @param yi The stating y coordinate of the wall
   * @param yf The ending y coordinate of the wall
   * @param x	The x coordinate of the wall
  ...
  /**
   * Sets a specified location to have a 1D north wall. If the specified cell already has a 1D east wall set in that location, then the cell
  ...
  /**
   * Sets a specified location to have a 1D east wall. If the specified cell already has a 1D north wall set in that location, then the cell
   ...
   * @param x the x coordinate of the location
   * @param y the y coordinate of the location
   * @param wallType the wall configuration for this location. 0 = no walls; 1 = complete cell wall/obstacle; 2 = 1D north wall; 3 = 1D east
  ...
  /**
   * Creates and returns a {@link burlap.behavior.singleagent.auxiliary.valuefunctionvis.ValueFunctionVisualizerGUI} object for a grid
   * world. The value of states will be represented by colored cells from red (lowest value) to blue (highest value). North-south-east-west
   ...
   * {@link burlap.behavior.singleagent.auxiliary.valuefunctionvis.ValueFunctionVisualizerGUI#initGUI()} on the returned object to start it.
   *
  ...
  public static class GridWorldModel implements FullStateModel {
     ...
     * Matrix specifying the transition dynamics in terms of movement directions. The first index indicates the action direction attempted
     * (ordered north, south, east, west) the second index indicates the actual resulting direction the agent will go (assuming there is no
     * wall in the way). The value is the probability of that outcome. The existence of walls does not affect the probability of the
     * direction the agent will actually go, but if a wall is in the way, it will affect the outcome. For instance, if the agent selects
     * north, but there is a 0.2 probability of actually going east and there is a wall to the east, then with 0.2 probability, the agent
     * will stay in place.
     */
    protected double[][] transitionDynamics;
  ...
  /**
   * Creates a visual explorer or terminal explorer. By default a visual explorer is presented; use the "t" argument to create terminal
   * explorer. Will create a 4 rooms grid world with the agent in lower left corner and a location in the upper right. Use w-a-s-d to move.
   *
