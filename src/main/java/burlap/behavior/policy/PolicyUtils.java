...
public class PolicyUtils {
  ...
  /**
   * Indicates whether rollout methods will decompose {@link burlap.behavior.singleagent.options.Option} selections into the primitive
   * {@link Action} objects they execute and annotate them with the name of the calling {@link burlap.behavior.singleagent.options.Option}
   * in the returned {@link Episode}. Default value is true.
   */
  public static boolean rolloutsDecomposeOptions = true;

  /**
   * Returns the probability of the policy taking action a in state s by searching for the action in the returned policy distribution from
   * the provided {@link EnumerablePolicy}.
   *
  ...
  /**
   * A helper method for defining deterministic policies. This method relies on the {@link Policy#action(State)} method being implemented
   * and will return a list of {@link ActionProb} objects with a single instance: the result of the {@link Policy#action(State)} method with
  ...
  /**
   * This is a helper method for stochastic policies. If the policy is stochastic, then rather than having the policy define both the
   * {@link Policy#action(State)} method and {@link EnumerablePolicy#policyDistribution(State)} method, the objects needs to only define the
   * {@link EnumerablePolicy#policyDistribution(State)} method and the {@link Policy#action(State)} method can simply return the result of
   * this method to sample an action.
   *
   * @param p the {@link EnumerablePolicy}
  ...
  /**
   * This method will return the an episode that results from following the given policy from state s. The episode will terminate when the
   * policy reaches a terminal state.
   *
  ...
  /**
   * This method will return the an episode that results from following the given policy from state s. The episode will terminate when the
   * policy reaches a terminal state or when the number of steps surpasses maxSteps.
   *
   * @param p the {@link Policy} to roll out
   * @param s the state from which to roll out the policy
  ...
  /**
   * Follows the policy in the given {@link burlap.mdp.singleagent.environment.Environment}. The policy will stop being followed once a
   * terminal state in the environment is reached.
   *
   * @param p the {@link Policy}
  ...
  /**
   * Follows the policy in the given {@link burlap.mdp.singleagent.environment.Environment}. The policy will stop being followed once a
   * terminal state in the environment is reached or when the provided number of steps has been taken.
   *
   * @param p the {@link Policy}
   * @param env The {@link burlap.mdp.singleagent.environment.Environment} in which this policy is to be evaluated.
  ...
  /**
   * Follows this policy for one time step in the provided {@link burlap.mdp.singleagent.environment.Environment} and records the
   * interaction in the provided {@link Episode} object. If the policy selects an {@link burlap.behavior.singleagent.options.Option}, then
   * how the option's interaction in the environment is recorded depends on the {@link #rolloutsDecomposeOptions} flag. If
   * {@link #rolloutsDecomposeOptions} is false, then the option is recorded as a single action. If it is true, then the individual
   * primitive actions selected by the environment are recorded.
   *
   * @param p the {@link Policy}
