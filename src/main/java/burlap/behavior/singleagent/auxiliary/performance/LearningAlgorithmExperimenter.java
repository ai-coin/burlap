 ...
 * {@link burlap.mdp.singleagent.environment.Environment} in which to perform the experiments, a number of trials, the length of the trials,
 * and an array of learning agent factories used to generated agent instances and compare their performance. The
 * {@link burlap.mdp.singleagent.environment.Environment} may optionally implement the
 * {@link burlap.behavior.singleagent.auxiliary.performance.ExperimentalEnvironment} interface which will let this class to tell the
 * {@link burlap.mdp.singleagent.environment.Environment} whenever experiments with a new agent class (defined by an
 * {@link burlap.behavior.singleagent.learning.LearningAgentFactory} is begun). The length of the trials by default is assumed to be in
 * episodes, but it may also be changed to indicate length in total number of steps using the
 * {@link #toggleTrialLengthInterpretation(boolean)} method.
 * <p>
 * Performance results are displayed in plots using the {@link PerformancePlotter} class, but visualization may also be disabled with the
 * {@link #toggleVisualPlots(boolean)} method. Results may be saved to csv files after the experiment is complete.
 * <p>
 * The purpose of the experimenter is to test an agent for a specified number of trials. At the beginning of each trial, a new agent is
 * generated using the designated LearningAgentFactory and is used for the specified trial length. After all trials are complete for an
 * agent, the next agent is tested. Note that immediately before an agent is generated from an agent factory, the performance plotter is
 * temporarily frozen from collecting data until the new agent is returned. This allows agent factories to perform offline learning before
 * returning a new agent in the same domain without affecting the experimenter results.
 * <p>
 * By default the cumulative reward per step will be plotted and if more than one trial is specified, the both the most recent trail and the
 * trial average plot will be shown. If only one trial is specified, then only the most recent trial plot will be shown. To control the
 * kinds of plots displayed use the {@link #setUpPlottingConfiguration(int, int, int, int, TrialMode, PerformanceMetric...)} method.
 *
...
public class LearningAlgorithmExperimenter {
  ...
  /**
   * The {@link EnvironmentServer} that wraps the test {@link burlap.mdp.singleagent.environment.Environment} and tells a
   * {@link burlap.behavior.singleagent.auxiliary.performance.PerformancePlotter} about the individual interactions.
   */
  ...
  /**
   * The delay in milliseconds between automatic refreshes of the plots
   */
  protected int plotRefresh = 1000;

  /**
   * The significance value for the confidence interval in the plots. The default is 0.05 which correspodns to a 95% CI
   */
  ...
  /**
   * Sets up the plotting configuration.
   *
   ...
   * &lt;pathAndBaseNameToUse&gt;Episodes.csv. The step-wise data will. If the experimenter as not been run, then nothing will be saved and
   * a warning message will be printed to indicate as such. be saved to the file &lt;pathAndBaseNameToUse&gt;Steps.csv
   *
  ...
  /**
   * Writes an episode-wise data to a csv file. If the file path does not include the .csv extension, it will automatically be added. If the
   * experimenter as not been run, then nothing will be saved and a warrning message will be printed to indicate as such.
   *
  ...
  /**
   * Writes an step-wise data to a csv file. If the file path does not include the .csv extension, it will automatically be added. If the
   * experimenter as not been run, then nothing will be saved and a warrning message will be printed to indicate as such.
   *
   * @param filePath the path to the csv file to write to.
   */
