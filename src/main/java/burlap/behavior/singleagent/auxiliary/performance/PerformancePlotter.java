...
/**
 * This class is an action observer used to collect and plot performance data of a learning agent either by itself or against another
 * learning agent. There are six possible performance metrics that can be used to plot performance, as specified in the
 * {@link PerformanceMetric} enumerator. A plot showing the most recent "trial" of an agent can be displayed, the average of the metric over
 * all trials with confidence intervals, or both may be displayed; which plots are shown is specfied by the {@link TrialMode} enumerator.
 * Any subset of these metrics may be displayed in any order specified by the user and plots are displayed in a matrix format with a maximum
 * number of columns that are filled out first. If the number of plots would cause a window height larger than a maximimum specified, then
 * the plots are placed in a scroll view.
 * <p>
 * The way this class should be used is the constructor is first called. Then the {@link #startGUI()} method. A trial indicates a single
 * evaluation of a learning algorithm for some number of steps or episodes. Multiple trials are used to produce the average trial plots.
 * Each trial should reinitialize the learning algorithm so that it learns from scratch. At the start of each trial the
 * {@link #startNewTrial()} method should be called. At the end of each episode in a trial, the {@link #endEpisode()} method should be
 * called. At the end of a trial the {@link #endTrial()} method should be called. When all trials for the current agent are complete and a
 * new agent is to tested to be compared, the {@link #startNewAgent(String)} method should be called, providing the name of the new agent to
 * be tested. Since the constructor takes the name of the first agent, this method does not have to be called for the first agent. When all
 * testing for all agents is complete, a call to the {@link #endTrialsForCurrentAgent()} method should be made.
 * <p>
 * To ensure proper use of this class, it is highly reccomended that the {@link LearningAlgorithmExperimenter} class is used, since it
 * handles all of these method calls behind the scenes.
 * <p>
 ...
 * metric (cumulaitve reward by step) for all agents and trials. Another will produce all the episode-wise performance metric data. This
 * data can be produced regardless of which metrics you requested to be plotted.
 * <p>
 * Note that the plots that are created have a number of interactive options. Try right-clicking on them to see the list of things you can
 * modfiy in the GUI.
 *
 *
...
public class PerformancePlotter extends JFrame implements EnvironmentObserver {
   ...
   * @param trialMode which plots to use; most recent trial, average over all trials, or both. If both, the most recent plot will be
   * inserted into the window first, then the average.
   * @param metrics the metrics that should be plotted. The metrics will appear in the window in the order that they are specified (columns
   * first)
   */
  ...
  /**
   * Sets the significance used for confidence intervals. The default is 0.05, which corresponds to a 95% confidence interval.
   *
  ...
  /**
   * Informs the plotter that data collecton for a new agent should begin. If the current agent is already set to the agent name provided,
   * then a warning message is printed and nothing changes.
   *
  ...
  /**
   * Informs the plotter that all data for all agents has been collected. Will also cause the average plots for the last agent's data to be
   * plotted.
   */
  ...
  /**
   * Writes the step-wise and episode-wise data to CSV files. The episode-wise data will be saved to the file
   * &lt;pathAndBaseNameToUse&gt;Episodes.csv. The step-wise data will be saved to the file &lt;pathAndBaseNameToUse&gt;Steps.csv
   *
  ...
  /**
   * Writes the step-wise data to a csv file. If the file path does not include the .csv extension, it will automatically be added.
   *
  ...
  /**
   * Writes the episode-wise data to a csv file. If the file path does not include the .csv extension, it will automatically be added.
   *
   * @param filePath the path to the csv file to write to.
   */
  ...
  /**
   * Adds the most recent trial (if enabled) chart and trial average (if enabled) chart into the provided container. The GridBagConstraints
   * will aumatically be incremented to the next position after this method returns.
   *
   ...
   * @param title the title to label thep plot; if average trial plots are enabled the word "Average" will be prepended to the title for the
   * average plot.
  ...
  /**
   * Increments the x-y position of a constraint to the next position. If there are still free columns in the current row, then the next
   * position in the next column; otherwise a new row is started.
   *
