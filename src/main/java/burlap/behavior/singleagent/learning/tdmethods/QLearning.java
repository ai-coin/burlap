...
/**
 * Tabular Q-learning algorithm [1]. This implementation will work correctly with Options [2]. The implementation can either be used for
 * learning or planning, the latter of which is performed by running many learning episodes in succession in a
 * {@link burlap.mdp.singleagent.environment.SimulatedEnvironment}. If you are going to use this algorithm for planning, call the
 * {@link #initializeForPlanning(int)} method before calling {@link #planFromState(State)}. The number of episodes used for planning can be
 * determined by a threshold maximum number of episodes, or by a maximum change in the Q-function threshold.
 * <p>
 * By default, this agent will use an epsilon-greedy policy with epsilon=0.1. You can change the learning policy to anything with the
 * {@link #setLearningPolicy(burlap.behavior.policy.Policy)} policy.
 * <p>
 * If you want to use a custom learning rate decay schedule rather than a constant learning rate, use the
 ...
 * <p>
 * 2. Sutton, Richard S., Doina Precup, and Satinder Singh. "Between MDPs and semi-MDPs: A framework for temporal abstraction in
 * reinforcement learning." Artificial intelligence 112.1 (1999): 181-211.
 *
...
public class QLearning extends MDPSolver implements QProvider, LearningAgent, Planner {
  ...
  /**
   * Initializes Q-learning with 0.1 epsilon greedy policy, the same Q-value initialization everywhere, and places no limit on the number of
   * steps the agent can take in an episode. By default the agent will only save the last learning episode and a call to the
   * {@link #planFromState(State)} method will cause the valueFunction to use only one episode for planning; this should probably be changed
   * to a much larger value if you plan on using this algorithm as a planning algorithm.
   *
  ...
  /**
   * Initializes Q-learning with 0.1 epsilon greedy policy, the same Q-value initialization everywhere. By default the agent will only save
   * the last learning episode and a call to the {@link #planFromState(State)} method will cause the valueFunction to use only one episode
   * for planning; this should probably be changed to a much larger value if you plan on using this algorithm as a planning algorithm.
   *
   * @param domain the domain in which to learn
   * @param gamma the discount factor
   * @param hashingFactory the state hashing factory to use for Q-lookups
   ...
   * agent (as it should be), you may need to set the policy to point to this object after call this constructor; the constructor will not
   * do this automatically in case it was by design to use the policy that was learned in some other domain. By default the agent will only
   * save the last learning episode and a call to the {@link #planFromState(State)} method will cause the valueFunction to use only one
   * episode for planning; this should probably be changed to a much larger value if you plan on using this algorithm as a planning
   * algorithm.
   *
   * @param domain the domain in which to learn
   * @param gamma the discount factor
  ...
  /**
   * Initializes the algorithm. Note that if the provided policy is derived from the Q-value of this learning agent (as it should be), you
   * may need to set the policy to point to this object after call this constructor; the constructor will not do this automatically in case
   * it was by design to use the policy that was learned in some other domain. By default the agent will only save the last learning episode
   * and a call to the {@link #planFromState(State)} method will cause the valueFunction to use only one episode for planning; this should
   * probably be changed to a much larger value if you plan on using this algorithm as a planning algorithm.
   *
   * @param domain the domain in which to learn
   * @param gamma the discount factor
   * @param hashingFactory the state hashing factory to use for Q-lookups
  ...
  /**
   * Initializes the algorithm. By default the agent will only save the last learning episode and a call to the
   * {@link #planFromState(State)} method will cause the valueFunction to use only one episode for planning; this should probably be changed
   * to a much larger value if you plan on using this algorithm as a planning algorithm.
   *
   * @param domain the domain in which to learn
  ...
  /**
   * Sets the {@link RewardFunction}, {@link burlap.mdp.core.TerminalFunction}, and the number of simulated episodes to use for planning
   * when the {@link #planFromState(State)} method is called.
   *
  ...
  /**
   * Sets a max change in the Q-function threshold that will cause the {@link #planFromState(State)} to stop planning when it is achieved.
   *
  ...
  /**
   * Sets whether the primitive actions taken during an options will be included as steps in returned EpisodeAnalysis objects. The default
   * value is true. If this is set to false, then EpisodeAnalysis objects returned from a learning episode will record options as a single
  ...
  /**
   * Returns the {@link QLearningStateNode} object stored for the given hashed state. If no {@link QLearningStateNode} object. is stored,
   * then it is created and has its Q-value initialize using this objects {@link burlap.behavior.valuefunction.QFunction} data member.
   *
  ...
  /**
   * Plans from the input state and then returns a {@link burlap.behavior.policy.GreedyQPolicy} that greedily selects the action with the
   * highest Q-value and breaks ties uniformly randomly.
   *
  ...
  /**
   * Writes the q-function table stored in this object to the specified file path. Uses a standard YAML approach, which means the
   * HashableState and underlying Domain states must have JavaBean like properties; i.e., have a default constructor and getters and setters
   * (or public data members) for all relevant fields.
   *
  ...
  /**
   * Loads the q-function table located on disk at the specified path. Expects the file to be a Yaml representation of a Java {@link Map}
   * from {@link HashableState} to {@link QLearningStateNode}.
   *
