...
/**
 * The abstract super class to use for various MDP solving algorithms, including both planning and learning algorithms. It implements the
 * {@link burlap.behavior.singleagent.MDPSolverInterface} and provides the common data members and method implementations that most all
 * algorithms will need to use and provides methods for manipulating them that are common.
 *
...
public abstract class MDPSolver implements MDPSolverInterface {
  ...
  /**
   * The list of actions this solver can use. May include non-domain specified actions like
   * {@link burlap.behavior.singleagent.options.Option}s.
   */
  protected List<ActionType> actionTypes;

  /**
   * A mapping to internal stored hashed states ({@link burlap.statehashing.HashableState}) that are stored. Useful since two identical
   * states may have different object instance name identifiers that can affect the parameters in GroundedActions.
   */
  ...
  /**
   * Returns all applicable actions in the provided state for all the actions that this MDP Solver can use. If this solver had
   * {@link ActionType}s not included in the domain added (e.g., types for {@link burlap.behavior.singleagent.options.Option}s), this method
