...
/**
 * This class allows you act as the agent by choosing actions in an {@link burlap.mdp.singleagent.environment.Environment}. States are
 * conveyed to the user through a 2D visualization and the user specifies actions by either pressing keys that are mapped to actions or by
 * typing the actions into the action command field. Action parameters in the action field are specified by space delineated input. For
 * instance: "stack block0 block1" will cause the stack action to called with action parameters block0 and block1. The ` key causes the
 * environment to reset.
 * <p>
 * Additionally, the VisualExplorer also creates an associated instance of an {@link burlap.shell.EnvironmentShell} that you can access
 * using the "Show Shell" button. You can use the shell to modify the state (assuming the input
 * {@link burlap.mdp.singleagent.environment.Environment} implements {@link StateSettableEnvironment}, record trajectories that you make in
 * the environment, and any number of other tasks. The shell's "programs," specified with {@link burlap.shell.command.ShellCommand}
 * instances, may also be expanded so that you can create your own runtime tools. See the {@link burlap.shell.BurlapShell} and
 * {@link burlap.shell.EnvironmentShell} Java doc for more information on how to use them. If you need access to the shell instance, you can
 * get it with the {@link #getShell()} method. You can also set shell commands to be executed when a key is pressed in the visualization.
 * Set them with the {@link #addKeyShellCommand(String, String)} method.
 * <p>
 * A special shell command, "livePoll" is automatically added to the shell, that allows the user to set the visualizer to auto poll the
 * environment at a fixed interval for the state and display it. This is useful when the environment can evolve independent of agent action
 * or explicit shell commands. See it's help in the shell (-h option) for more information on it.
 *
...
public class VisualExplorer extends JFrame implements ShellObserver {
   ...
   * as the environment with which to interact. The created {@link burlap.mdp.singleagent.environment.SimulatedEnvironment} will have a
   * {@link burlap.mdp.singleagent.common.NullRewardFunction} and {@link burlap.mdp.auxiliary.common.NullTermination} functions set.
   *
  ...
  public void init() {
    assert state != null;
    Environment env = new SimulatedEnvironment(domain, state);
    init(domain, env, painter, cWidth, cHeight);
  }
  ...
  /**
   * Starts a thread that polls this explorer's {@link burlap.mdp.singleagent.environment.Environment} every msPollDelay milliseconds for
   * its current state and updates the visualizer to that state. Polling can be stopped with the {@link #stopLivePolling()}. If live polling
   * is already running, this method call will only change the poll rate.
   *
  ...
  /**
   * Executes the provided {@link Action} in the explorer's environment and records the result if episodes are being recorded.
   *
   * @param ga the {@link Action} to execute.
   */
  public void executeAction(Action ga) {
