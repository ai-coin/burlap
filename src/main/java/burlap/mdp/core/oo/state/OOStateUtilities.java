...
public class OOStateUtilities {
  ...
  /**
   * Returns the value of an {@link ObjectInstance} variable, where the key is a {@link OOVariableKey} or a string representation of an
   * {@link OOVariableKey}.
   *
   * @param s the input state
  ...
  public static Map<String, List<ObjectInstance>> objectsByClass(OOState s) {
    ...
    Map<String, List<ObjectInstance>> obsByClass = new HashMap<>(classes.size());
    for (String className : classes) {
      List<ObjectInstance> obs = new ArrayList<>(s.objectsOfClass(className));
  ...
  public static Set<String> objectClassesPresent(OOState s) {
    ...
    Set<String> classes = new HashSet<>(obs.size());
  ...
  /**
   * A common approach for turning an {@link OOState} into a string representation. Useful for implementing {@link Object#toString()}
   * methods.
   *
   * @param s the input {@link OOState}
  ...
  /**
   * Returns all possible object assignments from a {@link OOState} for a given set of parameters that are typed to OO-MDP object classes.
   * For example, parameters for a {@link burlap.mdp.core.oo.propositional.PropositionalFunction} or
   * {@link burlap.mdp.core.oo.ObjectParameterizedAction}.
   *
   * @param s the {@link OOState}
   * @param paramClasses the object class type for a parameter array that needs object assignments
   * @param paramOrderGroups indicates the order group of a parameter. Parameters in the same order group can have their object assignments
   * shuffled without affecting the evaluation. By default, you probably can have a unique order group for each parameters
   * @return the list of all object assignments, where each object assignment is a list of object names of equal size to the input
   * parameters.
   */
  public static List<List<String>> getPossibleBindingsGivenParamOrderGroups(OOState s, String[] paramClasses, String[] paramOrderGroups) {
    ...
    List<List<String>> currentBindingSets = new ArrayList<>();
  ...
  private static List<String> identifyUniqueClassesInParameters(String[] paramClasses) {
    ...
    for (String paramClasse : paramClasses) {
      if (!unique.contains(paramClasse)) {
        unique.add(paramClasse);
      }
    }
    return unique;
  }
  ...
  private static void getPossibleRenameBindingsHelper(List<List<String>> res, List<List<String>> currentBindingSets, int bindIndex,
    ...
    for (List<String> cb : combs) {
      ...
      List<List<String>> nextBinding = new ArrayList<>(currentBindingSets.size());
   ...
   *
   * @param comboSets is a list of the bindings for each order group. For instance, if the order groups for each parameter were P, Q, P, Q,
   * R; then there would be three lists
  ...
  private static List<String> getBindingFromCombinationSet(List<List<String>> comboSets, List<String> orderGroupAssociatedWithSet, String[] orderGroups) {
    ...
    List<String> res = new ArrayList<>(orderGroups.length);
  ...
  private static int getNumOccurencesOfClassInParameters(String className, String[] paramClasses) {
    int num = 0;
    for (String paramClasse : paramClasses) {
      if (paramClasse.equals(className)) {
        num++;
      }
    }
    return num;
  ...
  private static List<ObjectInstance> objectsMatchingClass(List<ObjectInstance> objects, String className) {
    List<ObjectInstance> filtered = new ArrayList<>(objects.size());
  ...
  private static List<String> getListOfBindingsFromCombination(List<ObjectInstance> objects, int[] comb) {
    List<String> res = new ArrayList<>(comb.length);
  ...
  private static List<ObjectInstance> objectListDifference(List<ObjectInstance> objects, List<String> toRemove) {
    List<ObjectInstance> remaining = new ArrayList<>(objects.size());
  ...
  /**
   * Iterates through combinations. Modified code from: http://compprog.wordpress.com/tag/generating-combinations/
   *
